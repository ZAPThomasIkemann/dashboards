...
Alle Zugangsdaten (OpenAI, Anthropic, DataForSEO) liegen in `/workspace/secrets.php` — diese Datei ist in `.gitignore` und erscheint **nie im Repository**.

- Vorlage: `secrets.example.php` (committed, ohne echte Keys)
- Echte Keys: `secrets.php` (gitignored, lokal anlegen)
- Auf dem Produktionsserver muss `secrets.php` im gleichen Pfad relativ zum Repository-Root abgelegt werden

**AI-Provider wechseln:** `AI_PROVIDER` in `secrets.php` auf `'openai'` oder `'anthropic'` setzen. Alle Workflow-Skripte nutzen automatisch den richtigen Anbieter via `dmc/api/ai.php`.

Verfügbare Anthropic-Modelle (verifiziert): `claude-sonnet-4-5` (fast), `claude-opus-4-5` (strong).
